...
const ItDashboard = () => {
  ...
  if (token) {
    try {
      ...
      user.username = decoded.username || decoded.name || "IT";
  ...
  const handleMarkAsRead = (id) => {
    ...
    fetch(`/api/it/notifications/${id}/read`, { method: "POST" });
  };
  const handleLogout = () => {
    ...
    <div className="it-dashboard-container">
      <aside className="it-sidebar">
        ...
        <nav className="it-sidebar-nav">
          ...
          {features.map((f) => (
            <a key={f} href="#" className="it-sidebar-link">
              {f}
            </a>
          ))}
        </nav>
